...
class WP_REST_Order_Controller extends WP_REST_Controller
{
    ...
    public function register_routes()
    {
        ...
        register_rest_route($this->namespace, '/' . $this->rest_base . '/(?P<id>[\d_U]+)', array(
            array(
                'methods' => WP_REST_Server::READABLE,
...
//        }
    ...
    public function create_item($request)
    {
        ...
        if ($create instanceof WP_Error) {
            ...
            return new WP_Error('create_order_failed', __('Create order failed', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }
    ...
    {
        if (!is_user_logged_in() && strpos($request['id'],'U') !== 0) {
    ...
    public function get_item($request)
    {
        ...
        if ($order->product_id == '-9' || $order->product_id == '-8') {
          if (strpos($order->trade_no,'Pay') !== 0) {
                return tt_api_fail(__('The order has not been payed yet', 'tt'));
             }
          $manage_url = add_query_arg(array('oid' => $order->order_id, 'spm' => wp_create_nonce('pay_result')), tt_url_for('payresult'));
        }else{
    ...
    public function update_item($request)
    {
        ...
        } elseif ($request->get_param('new_price')) {
          if(current_user_can('administrator')){
            $update = tt_update_order($order_id, array('order_total_price' => $request->get_param('new_price')), array('%f'));
            $order = tt_get_order($order_id);
            $user = get_user_by('id', $order->user_id);
            $order_url = tt_url_for('my_order', $order_id);

            $blog_name = get_bloginfo('name');
            $admin_email = get_option('admin_email');
            $order_status_text = tt_get_order_status_text($order->order_status);
            $subject = sprintf(__('「%s」 商店订单价格变更提醒', 'tt'), $blog_name);
            $args = array(	
                'blogName' => $blog_name,
            ...
            return array('success' => false,'message' =>'你没有权限修改价格');
          }
        } elseif ($request->get_param('checkout') == 'true') {
            ...
            if ($order->order_currency == 'credit') {
                ...
                $pay_method = $request->get_param('payMethod');
	            // 考虑是否使用现金余额
                $use_balance = $request->get_param('useBalance') == 'true';
                $total = $order->order_total_price;
                if ($use_balance) {
                    $balance = tt_get_user_cash();
                    $pay_balance = tt_cash_pay(min($balance, $total), $order->product_name);
                    if ($pay_balance) {
                ...
                if ($total < 0.01) {
                    ...
                    $url = add_query_arg(array('oid' => $order->order_id, 'spm' => wp_create_nonce('pay_result')), tt_url_for('payresult'));
                    return tt_api_success('', array('data' => array( // 返回payment gateway url
                        'orderId' => $order_id,
                        'url' => $url
                    )));
                ...
                switch ($pay_method) {
                    case 'alipay' :
                        if ($order->order_total_price - $total >= 0.01) {
                            ...
                            tt_update_order($order_id, array('order_total_price' => $total), array('%f'));
                        }
                    ...
                    case 'aliqrpay' :
                        ...
                        $url = add_query_arg(array('oid' => $order_id, 'channel' => 'aliqrpay'), tt_url_for('aliqrpay'));
                        return tt_api_success('', array('data' => array( // 返回payment gateway url
                            'orderId' => $order_id,
                            'url' => $url
                        )));
                    case 'kpay_alipay' :
                        ...
                        $url = add_query_arg(array('oid' => $order_id, 'channel' => 'kpay_alipay'), tt_url_for('kpay'));
                        return tt_api_success('', array('data' => array( // 返回payment gateway url
                            'orderId' => $order_id,
                            'url' => $url
                        )));
                    case 'kpay_wxpay' :
                        ...
                        $url = add_query_arg(array('oid' => $order_id, 'channel' => 'kpay_wxpay'), tt_url_for('kpay'));
                        return tt_api_success('', array('data' => array( // 返回payment gateway url
                            'orderId' => $order_id,
                            'url' => $url
                        )));
                    case 'kpay_qqpay' :
                        ...
                        $url = add_query_arg(array('oid' => $order_id, 'channel' => 'kpay_qqpay'), tt_url_for('kpay'));
                        return tt_api_success('', array('data' => array( // 返回payment gateway url
                            'orderId' => $order_id,
                            'url' => $url
                        )));
                    case '1' :
                        ...
                        $url = add_query_arg(array('oid' => $order_id, 'channel' => '1'), tt_url_for('qrpay'));
                        return tt_api_success('', array('data' => array( // 返回payment gateway url
                            'orderId' => $order_id,
                            'url' => $url
                        )));
                    case '2' :
                        ...
                        $url = add_query_arg(array('oid' => $order_id, 'channel' => '2'), tt_url_for('qrpay'));
                        return tt_api_success('', array('data' => array( // 返回payment gateway url
                            'orderId' => $order_id,
                            'url' => $url
                        )));
                    case '3' :
                        ...
                        $url = add_query_arg(array('oid' => $order_id, 'channel' => '3'), tt_url_for('qrpay'));
                        return tt_api_success('', array('data' => array( // 返回payment gateway url
                            'orderId' => $order_id,
                            'url' => $url
                        )));
                    default : //qrcode
                        ...
                        return tt_api_success('', array('data' => array( // 直接返回扫码支付url,后面手动修改订单
                            ...
                            'orderId' => $order_id,
                            'url' => add_query_arg(array('oid' => $order_id), tt_url_for(tt_get_option('tt_pay_channel') === 'youzan' ? 'youzanpay' : 'youzanpay'))
                        )));
                }
            }
...
//                $format[] = '%s';
//            }
                $data['order_success_time'] = current_time('mysql');
                $format[] = '%s';
...
//            }
        ...
        elseif (!$update) {
            return new WP_Error('order_update_failed', __('Update order failed', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }
    ...
    public function delete_item($request)
    {
        ...
        if ($result instanceof WP_Error) {
            ...
            return new WP_Error('rest_order_cannot_delete', __('Delete order failed', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }
