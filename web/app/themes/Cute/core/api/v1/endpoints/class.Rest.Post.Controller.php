...
//            ),
    ...
    public function create_item( $request ) {
        ...
        if(strlen($title) < 10) {
            return new WP_Error('create_post_failed', __('The post title is too short or empty', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }

        $content = trim($request->get_param('content'));
        if(strlen($content) < absint(tt_get_option('tt_contribute_post_words_min', 100))) {
            return new WP_Error('create_post_failed', __('The post content is too short or empty', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }
        ...
        // 如果是直接发布的
        if ($action == 'publish') {
            do_action('tt_immediate_to_publish', $new_post);
        }
        ...
        if(!empty($sale_dl)) {
            update_post_meta($new_post, 'tt_sale_dl2', $sale_dl);
        }
    ...
    public function update_item( $request ) {
        ...
        if($request->get_param('onlyStatus')) {
            ...
            if($update_post instanceof WP_Error) {
                ...
                return new WP_Error('update_post_failed', __('Handle post failed', 'tt'), array('status' => tt_rest_authorization_required_code()));
            }
        ...
        $title = sanitize_text_field(trim($request->get_param('title')));
        if(strlen($title) < 10) {
            return new WP_Error('update_post_failed', __('The post title is too short or empty', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }

        $content = trim($request->get_param('content'));
        if(strlen($content) < 100) {
            return new WP_Error('update_post_failed', __('The post content is too short or empty', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }
        ...
        if(!empty($sale_dl)) {
            update_post_meta($update_post, 'tt_sale_dl2', $sale_dl);
        }
        //TODO email actions or notifications
