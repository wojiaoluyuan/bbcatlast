...
//            ),
    ...
    public function create_item( $request ) {
        ...
        if(strlen($title) < 10) {
            return new WP_Error('create_product_failed', __('The product name is too short or empty', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }

        $content = trim($request->get_param('content'));
        if(strlen($content) < 100) {
            return new WP_Error('create_product_failed', __('The product description is too short or empty', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }
    ...
    public function update_item( $request ) {
        ...
        if($request->get_param('onlyStatus')) {
            ...
            if($update_product instanceof WP_Error) {
                ...
                return new WP_Error('update_product_failed', __('Handle product failed', 'tt'), array('status' => tt_rest_authorization_required_code()));
            }
        ...
        $title = sanitize_text_field(trim($request->get_param('title')));
        if(strlen($title) < 10) {
            return new WP_Error('update_product_failed', __('The product name is too short or empty', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }

        $content = trim($request->get_param('content'));
        if(strlen($content) < 100) {
            return new WP_Error('update_product_failed', __('The product description is too short or empty', 'tt'), array('status' => tt_rest_authorization_required_code()));
        }
