...
abstract class Open{
    ...
    protected function saveOpenInfoAndProfile($user_id, $data){
        ...
        if(static::$_type === 'weixin'){
            update_user_meta($user_id, 'tt_weixin_avatar', set_url_scheme($data['headimgurl'], 'https'));
     ...
     */
    protected function openSignUp($user_login, $password, $oauth=''){
        // 获取缓存的OAuth数据
        ...
        if($user) {
            if(!wp_check_password( $password, $user->data->user_pass, $user->ID)) {
                return new WP_Error('unmatch_login_pass', __('The username is exist, please provide correct password for it if owned by you or choose another username', 'tt'), array('status' => 403));
            }
            // 更新用户数据
            ...
            $is_new = false;
        }elseif($reg_ver_option){
            // 发送注册邮件
            $activation_link = tt_generate_registration_activation_link($user_login, $user_login, $password, $oauth, $this->_data_transient_key);
            tt_async_mail('', $user_login, __('Your account activation link', 'tt'), array('email' => $user_login, 'name' => $user_login, 'link' => $activation_link), 'register-confirm');
        }else{
            // 开放平台连接并需要新建一个本地用户绑定
     ...
     * @return  bool|string|WP_Error
     */
    public function openHandleLast($user_login, $password, $is_get_data = true, $data_cache_key = '', $oauth=''){
        ...
        return $this->openSignUp($user_login, $password, $oauth);
    }
...
class OpenWeiXin extends Open{
    ...
    protected static $_openkey_option_name = 'tt_weixin_openkey';

    protected static $_opensecret_option_name = 'tt_weixin_opensecret';
