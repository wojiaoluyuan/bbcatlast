...
class Utils{
     ...
     * @static
     * @access public
     * @param bool  $strict   是否采用精确鉴别,粗略判断将获得更快的速度
     *              default false
     * @return bool
     */
    ...
    public static function isPhone(){
        ...
        foreach ($mobileHeaders as $mobileHeader => $matchType){
            if (is_array($matchType['matches'])) {
                foreach ($matchType['matches'] as $_match) {
                    if (strpos($httpHeaders[$mobileHeader], $_match) !== false) {
                        return true;
                    }
                }
                //return false;
        ...
        foreach ($phoneDeviceRegxs as $regex) {
            if (empty($regex)) {
                continue;
            }
            $match = (bool) preg_match(sprintf('#%s#is', $regex), $userAgent);
            if ($match) {
                return true;
            }
        }
    ...
    public static function isTablet(){
        ...
        $userAgent = $_SERVER['HTTP_USER_AGENT'];
        foreach ($tabletDeviceRegxs as $regex){
            $match = (bool) preg_match(sprintf('#%s#is', $regex), $userAgent);
            if ($match) {
                return true;
            }
     ...
     * @static
     * @access public
     * @param int/string $markTime      待处理的时间戳或时间字符串
     *                   type int       时间戳
     *                   type string    时间字符串
     * @return string
     */
    public static function getTimeDiffString($markTime){
        date_default_timezone_set ('ETC/GMT');  //TODO timezone issue
        if(is_string($markTime)){
            $markTime = strtotime($markTime);
        }
        if (!is_integer($markTime)){
            return '';
        }
        $diff = time()-$markTime;
        if($diff < 10){
        ...
        $cases = array (
            12 * 30 * 24 * 60 * 60  =>  __('年前', 'tt').' ('.date('Y-m-d', $markTime).')',
            30 * 24 * 60 * 60       =>  __('个月前', 'tt').' ('.date('m-d', $markTime).')',
            7 * 24 * 60 * 60        =>  __('周前', 'tt').' ('.date('m-d', $markTime).')',
            24 * 60 * 60            =>  __('天前', 'tt'),
            60 * 60                 =>  __('小时前', 'tt'),
            60                      =>  __('分钟', 'tt'),
            1                       =>  __('秒前', 'tt')
        );
        foreach ($cases as $secs => $str) {
            $d = $diff / $secs;
            if ($d >= 1) {
                $r = round($d);
                return $r . $str;
     ...
     * @param int    $from      截取起始字符索引,从0开始
     * @param int    $len       截取长度
     * @param string $suffix    添加后缀
     * @return string
     */
    public static function cutStr($str, $from, $len, $suffix=''){
        $newStr = preg_replace('#^(?:[\x00-\x7F]|[\xC0-\xFF][\x80-\xBF]+){0,'.$from.'}'.
     ...
     * @static
     * @access  public
     * @param   string  $url    原始HTTP链接,不带协议头的默认为HTTP协议
     * @return  string
     */
    public static function getHttpsUrl($url){
        $pattern = '/^(http:\/\/)?([\w-_.]+(\/[\w-_]+)*\/?)$/i';
        $replacement = 'https://$2';
