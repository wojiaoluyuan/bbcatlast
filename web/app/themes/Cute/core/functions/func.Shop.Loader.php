...
function tt_get_specified_member_product_price($product_id, $member = 0) {
    $currency = get_post_meta( $product_id, 'tt_pay_currency', true) ? 'cash' : 'credit';
    $price = $currency == 'cash' ? sprintf('%0.2f', get_post_meta($product_id, 'tt_product_price', true)) : (int)get_post_meta($product_id, 'tt_product_price', true);
...
function tt_get_product_pay_content($product_id, $html = true)
{
    ...
    $show = tt_check_user_has_buy_product($product_id, $user_id);
    if (!$show) {
...
{
    $pay_method = tt_get_option('tt_pay_channel', 'alipay');
...
function tt_get_order_by_name($qr_name) {
    global $wpdb;
    $prefix = $wpdb->prefix;
    $orders_table = $prefix . 'tt_orders';
    $qr_name = explode('-站内订单-',$qr_name);
    $order_id = $qr_name[1];
    $order = $wpdb->get_row(sprintf("SELECT * FROM $orders_table WHERE `id`='%d'", $order_id));
    return $order;
}
...
function tt_create_invite_order($product_id, $product_name = '', $order_quantity = 1, $parent_id = 0)
{
    $user_id = '0';
    $order_id = 'U'.tt_generate_order_num();
    $order_time = current_time('mysql');
    $currency = 'cash';
    $order_price = tt_get_option('tt_k_invite_price');
...
function tt_create_donate_order($product_id, $product_name = '', $order_quantity = 1, $parent_id = 0)
{
    $user_id = get_current_user_id() ? get_current_user_id() : '0';
    $order_id = 'U'.tt_generate_order_num();
    $order_time = current_time('mysql');
    $currency = 'cash';
    $order_price = tt_get_option('tt_k_donate_price');
...
function tt_create_combine_orders($product_ids, $order_quantities)
{
    ...
    if (!$insert) {
        return new WP_Error('create_order_failed', __('Create combine order failed', 'tt'), array('status' => 403));
    }
    $insert_id = $wpdb->insert_id;
    // 创建子订单
    $total_price = 0;
    foreach ($product_ids as $key => $product_id) {
        ...
        if (!$sub_order) {
            return new WP_Error('create_order_failed', __('Create combine order failed', 'tt'), array('status' => 403)); //TODO distinguish
        }
...
function tt_delete_order_by_order_id($order_id){
    ...
    $order = tt_get_order($order_id);
    if(!$order){
        return new WP_Error('order_not_exist', __('The order is not exist', 'tt'), array('status' => 403));
    }

    if($order->user_id != $user_id && !current_user_can('edit_users')){
        return new WP_Error('delete_order_denied', __('You are not permit to delete this order', 'tt'), array('status' => 403));
    }
    ...
    if($parent_id == -1){ // 这是一个合并订单
        ...
        foreach ($sub_orders as $sub_order){
            tt_update_order($sub_order->order_id, array('deleted' => 1, 'deleted_by' => $user_id), array('%d', '%d'));
        }
    }
...
{
    if (!tt_get_option('tt_order_events_notify', true)) {
        return;
    }
    $order = tt_get_order($order_id);
    if (!$order || $order->product_id == -9 || $order->product_id == -8) {
        return;
    }
    ...
    $args = array(
        'blogName' => $blog_name,
        'buyerName' => $user->display_name ? $user->display_name : 'System',
        'orderUrl' => $order_url,
        'adminEmail' => $admin_email,
        'productName' => $order->product_name, //TODO suborders
        'orderId' => $order_id,
        'orderTotalPrice' => $order->order_currency == 'credit' ? sprintf(__('%d Credits', 'tt'), $order->order_total_price) : sprintf(__('%0.2f YUAN', 'tt'), $order->order_total_price),
...
function tt_continue_pay($order_id)
{
    ...
    if ($order->order_currency == 'credit') {
        ...
        return new WP_Error('continue_pay_failed', __('Some error happened when continue the payment', 'tt'), array('status' => 403));
    } else { // 现金支付
        ...
        switch ($pay_method){
            case 'alipay':
                return tt_api_success('', array('data' => array( // 返回payment gateway url
                    'orderId' => $order_id,
                    'url' => add_query_arg(array('oid' => $order_id, 'spm' => $checkout_nonce), tt_url_for('checkout'))
                )));
            default: //qrcode
...
function tt_maintain_orders($days)
{
    ...
    $sql = sprintf("SELECT * FROM $orders_table WHERE `order_status`=%d AND `deleted`=0 AND `parent_id`=0 AND `order_time`<=DATE_ADD(NOW(), INTERVAL %d HOUR) ORDER BY `id` DESC LIMIT %d OFFSET %d", OrderStatus::WAIT_PAYMENT, (-1) * $days * 24, 100, 0);
    $results = $wpdb->get_results($sql);
...
function tt_add_coupon($code, $type = 'once', $discount = 0.90, $begin_date, $expire_date/*, $unavailable_products = '', $unavailable_product_cats = ''*/)
{
    if (!current_user_can('edit_users')) {
        return new WP_Error('no_permission', __('You do not have the permission to add a coupon', 'tt'), array('status' => 403));
    }
    //检查code重复
    global $wpdb;
    $prefix = $wpdb->prefix;
    ...
    if ($exist) {
        return new WP_Error('exist_coupon', __('The coupon code is existed', 'tt'), array('status' => 403));
    }
...
{ // $data must be array( 'column1' => 'value1', 'column2' => 'value2') type
    $count = count($data);
    $format = array();
    for ($i = 0; $i < $count; ++$i) {
        $format[] = '%s';
...
{
    if (!wp_next_scheduled('tt_orders_maintenance_hourly_event')) {
        wp_schedule_event(1471708800, 'hourly', 'tt_orders_maintenance_hourly_event');
    }
}
...
add_action('tt_orders_maintenance_hourly_event', 'tt_orders_maintenance_do_this_daily');
