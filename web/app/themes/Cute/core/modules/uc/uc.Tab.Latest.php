            ...
            <?php foreach ($uc_latest_posts as $uc_latest_post) { ?>
                <article id="<?php echo 'post-' . $uc_latest_post['ID']; ?>" class="post type-post status-publish <?php echo 'format-' . $uc_latest_post['format']; ?>">
                    <div class="entry-thumb hover-scale">
                        <a href="<?php echo $uc_latest_post['permalink']; ?>"<?php echo tt_get_option('tt_enable_k_blank', false) ?  ' target="_blank"' : ''; ?>><img width="250" height="170" src="<?php echo LAZY_PENDING_IMAGE; ?>" data-original="<?php echo $uc_latest_post['thumb']; ?>" class="thumb-medium wp-post-image lazy" alt="<?php echo $uc_latest_post['title']; ?>"></a>
                        <?php echo $uc_latest_post['category']; ?>
                    </div>
                    <div class="entry-detail">
                        <header class="entry-header">
                            <h2 class="entry-title"><a href="<?php echo $uc_latest_post['permalink']; ?>" rel="bookmark"<?php echo tt_get_option('tt_enable_k_blank', false) ?  ' target="_blank"' : ''; ?>><?php echo $uc_latest_post['title']; ?></a><?php if($logged_user_id == $tt_author_id){?><span><a href="<?php echo tt_url_for('edit_post', $uc_latest_post['ID']); ?>" title="<?php _e('Edit Post', 'tt'); ?>"><i class="tico tico-new"></i></a> </span><?php } ?></h2>
                            <div class="entry-meta entry-meta-1">
                                <span class="author vcard"><a class="url" href="<?php echo $uc_latest_post['author_url']; ?>"<?php echo tt_get_option('tt_enable_k_blank', false) ?  ' target="_blank"' : ''; ?>><?php echo $uc_latest_post['author']; ?></a></span>
                                ...
                                <span class="comments-link text-muted"><i class="tico tico-comments-o"></i><a href="<?php echo $uc_latest_post['permalink'] . '#respond'; ?>"<?php echo tt_get_option('tt_enable_k_blank', false) ?  ' target="_blank"' : ''; ?>><?php echo $uc_latest_post['comment_count']; ?></a></span>
                            </div>
                        </header>
