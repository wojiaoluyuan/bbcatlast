            ...
            <?php foreach ($recent_comments as $recent_comment) { ?>
                <li class="comment">
                    <div class="comment-title" title="<?php echo $recent_comment['comment_date_diff']; ?>">
                        <span class="comment-author"><img class="avatar lazy" src="<?php echo LAZY_PENDING_AVATAR; ?>" data-original="<?php echo $recent_comment['author_avatar']; ?>"><?php echo tt_get_privacy_mail($recent_comment['author_name']); ?></span><?php _e('REVIEWED ON ', 'tt'); ?><a href="<?php echo $recent_comment['post_permalink']; ?>"<?php echo tt_get_option('tt_enable_k_blank', false) ?  ' target="_blank"' : ''; ?>><?php echo $recent_comment['post_title']; ?></a>
                    </div>
