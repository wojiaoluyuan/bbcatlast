...
class ShopProductVM extends BaseVM {
    ...
    protected function getRealData() {
        ...
        $info['sale_text'] = get_post_meta($the_product->ID, 'tt_product_sale_text',true);
        ...
        $info['author_url'] = get_author_posts_url($the_product->post_author); //TODO the link
        $info['author_thumb'] = tt_get_avatar($the_product->post_author, 'small');
        ...
        $info['thumb'] = tt_get_thumb($the_product, array('width' => 470, 'height' => 250, 'str' => 'medium'));
        ...
        // 实际价格
        $info['min_price'] = tt_get_specified_user_product_price($the_product->ID, get_current_user_id());
      
        // 月费会员价格
        $info['vip1_price'] = tt_get_specified_member_product_price($the_product->ID, 1);
        
        // 年费会员价格
        $info['vip2_price'] = tt_get_specified_member_product_price($the_product->ID, 2);
        
        // 永久会员价格
        $info['vip3_price'] = tt_get_specified_member_product_price($the_product->ID, 3);
        ...
        while ($relates_query->have_posts()) : $relates_query->the_post();
            ...
            $related_product['min_price'] = tt_get_specified_user_product_price($post->ID, get_current_user_id());
        ...
        while ($rand_query->have_posts()) : $rand_query->the_post();
            ...
            $rand_product['min_price'] = tt_get_specified_user_product_price($post->ID, get_current_user_id());
