...
abstract class WPAsyncTask {
    ...
    public function __construct( $auth_level = self::BOTH ) {
        ...
        if ( $auth_level & self::LOGGED_IN ) {
            add_action( "admin_post_tt_async_$this->action", array( $this, 'handle_postback' ) );
        }
        if ( $auth_level & self::LOGGED_OUT ) {
            add_action( "admin_post_nopriv_tt_async_$this->action", array( $this, 'handle_postback' ) );
        }
    }
    /**
    ...
    public function launch() {
        ...
        $data['action'] = "tt_async_$this->action";
