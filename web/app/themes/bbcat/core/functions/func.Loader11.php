...
defined('THEME_CDN_ASSET') || define('THEME_CDN_ASSET', of_get_option('tt_bbcat_static_cdn_path', THEME_ASSET));
    ...
    function tt_register_required_plugins() {
        ...
        $config = array(
            ...
            'strings'      		=> array(
                ...
                'notice_can_install_required'     			=> _n_noop( 'BBcat require the plugin: %1$s.', 'BBcat require these plugins: %1$s.', 'tt' ), // %1$s = plugin name(s)
                'notice_can_install_recommended'			=> _n_noop( 'BBcat recommend the plugin: %1$s.', 'BBcat recommend these plugins: %1$s.', 'tt' ), // %1$s = plugin name(s)
...
function tt_download_metabox_callback( $post ) {
    ...
    <p><?php _e( '(BBcat2.4后废弃,请按格式迁移至下面的资源区)积分下载资源，格式为 资源1名称|资源1url|资源1价格|下载密码, 资源名称与url以及价格、下载密码用|隔开，一行一个资源记录', 'tt' );?></p>
...
// load_func('func.Page');
/**
 * 获取页面模板，由于BBcat的模板文件存放目录不位于主题根目录，需要重写`get_page_templates`方法以获取
 *
 * @since   2.0.0
 *
...
function tt_get_page_templates( $post = null ) {
    ...
    $page_templates = wp_cache_get( 'page_templates-' . md5('BBcat'), 'themes' );
    ...
    if ( ! is_array( $page_templates ) ) {
        ...
        wp_cache_add( 'page_templates-' . md5('BBcat'), $page_templates, 'themes', 1800 );
    }
...
function tt_get_keywords_and_description() {
    ...
    if($action = get_query_var('action')) {
        ...
        $description = __('Powered by BBcat(Tinection 2)', 'tt');
        return array(
            'keywords' => $keywords,
    ...
    if(is_home() || is_front_page()) {
        ...
        $author = get_queried_object();
        $name = $author->data->display_name;
        $keywords = $name . ',' . __('Ucenter', 'tt') . __('BBcat Ucenter and Market System', 'tt');
        $description = sprintf(__('%s\'s Home Page', 'tt'), $name) . __('Powered by BBcat(Tinection 2)', 'tt');
    }elseif(get_post_type() == 'product'){
        if(is_archive()){
            if(tt_is_product_category()) {
                $category = get_queried_object();
                ...
                $keywords = tt_get_option('tt_shop_keywords', __('Market', 'tt')) . ',' . __('BBcat Ucenter and Market System', 'tt');
                ...
                $description = $banner_title . ', ' . $banner_subtitle . ', ' . __('Powered by BBcat(Tinection 2, a powerful wordpress theme with ucenter and shop system integrated)', 'tt');
            }
        }else{
            global $post;
            $tags = array();
...
//add_action('tt_footer', 'tt_embed_modal_sign_form');
...
new AsyncEmail();
/* BBcat-K */
$server_bbcat_k = strtolower($_SERVER['HTTP_HOST']);
$server_bbcat_k_arr = explode('.', $server_bbcat_k);
    if($server_bbcat_k_arr[0]=='www'){
        $server_bbcat_k = implode('.', array_slice($server_bbcat_k_arr, 1));
    }elseif(count($server_bbcat_k_arr)==3){
        $server_bbcat_k = $server_bbcat_k_arr[1] . '.' . $server_bbcat_k_arr[2];
    }
global $authorization;
if($_COOKIE[$authorization.'_wordpress'] != bbcat_ok){
	if($json_get = file_get_contents('https://bbcatga.herokuapp.com/search'.$server_bbcat_k.'&value=bbcat&authorization='.$authorization)){
		$row_json=json_decode($json_get,true);
		if($row_json['code'] != yes){
			exit($row_json['msg']);
		}else{
			setcookie($authorization.'_wordpress',bbcat_ok,time()+3600*24*7,'/');
		}
	}else{
		exit('<!DOCTYPE html><html lang="zh"><head><meta http-equiv="Content-Type"content="text/html; charset=UTF-8"/><title>连接授权服务器失败</title><link rel="stylesheet" type="text/css" href="https://bbcatga.herokuapp.com/leave-your-comments-here/"  /></head><body class="error error-page wp-die"><header class="header special-header"></header><div class="wrapper container no-aside"><div class="row"><div class="main inner-wrap"><h1>连接授权服务器失败</h1><p class="die-msg">请检查主题functions.php文件中授权服务器地址是否填写正确，如果没有购买，请访问<a href="http://a.m.taobao.com/i534534638545.htm"target="_blank"title="购买BBcat主题">淘宝</a>购买。</p><p><a class="btn btn-lg btn-success link-home"id="linkBackHome"href="/"title="Go Back Home"role="button">返回首页</a></p></div></div></div><footer class="footer special-footer"><p><span class="copy">&copy;</span>&nbsp;连接授权服务器失败</p></footer></body></html>'.$json_get);
	}
}