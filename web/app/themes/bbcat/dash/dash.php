...
function tt_clear_cache_on_admin_menu_bar( $wp_admin_bar ) {
    ...
    $args = array(
        ...
        'parent' => false,
        'href'  => wp_nonce_url( admin_url( 'admin.php?page=options-framework&bbcat_cache_empty=1' ), 'tt_clear_cache', 'tt_clear_cache_nonce' ),
...
function tt_clear_cache_callback_on_admin_menu_bar(){
    if(isset($_GET['tt_clear_cache_nonce']) && wp_verify_nonce($_GET['tt_clear_cache_nonce'], 'tt_clear_cache')) {
        if(isset($_GET['bbcat_cache_empty']) && $_GET['bbcat_cache_empty']==1) {
            tt_clear_all_cache();
...
function left_admin_footer_text($text) {
    $text = sprintf(__('<span id="footer-thankyou">Thanks for using %s to help your creation, %s theme style your website</span>', 'tt'), '<a href=http://cn.wordpress.org/ >WordPress</a>', '<a href="https://bbcatga.herokuapp.com">BBcat-K</a>');
    return $text;
}
